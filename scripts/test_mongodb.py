...
def run_test():
    ...
    if not uri:
        ...
        for item in simulated_db["inventory"]:
            if item["row"] == 1 and item["rack"] == 1:
                ...
                item["last_scanned"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
                updated = True
                break
    ...
    try:
        ...
        # Update
        test_col.update_one({"row": 1, "rack": 3}, {"$set": {"package_id": target_qr, "last_scanned": datetime.datetime.now(datetime.timezone.utc).isoformat()}})
